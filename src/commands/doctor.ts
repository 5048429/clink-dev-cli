...
export function registerDoctor(program: Command): void {
  program
    ...
    .action(async (options: { skipNetwork?: boolean; webhookUrl?: string }, command: Command) => {
      ...
      if (!options.skipNetwork && config.apiKey) {
        try {
          ...
          checks.push({ name: "api_connectivity", status: "fail", message: (error as Error).message });
        }
      }
      ...
      if (options.webhookUrl) {
        if (!config.webhookSigningKey) {
          ...
          try {
            ...
            checks.push({ name: "webhook_simulation", status: "fail", message: (error as Error).message });
          }
        }
      }
