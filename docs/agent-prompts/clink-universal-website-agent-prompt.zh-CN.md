...
- 云端托管平台、低代码平台、云 IDE 或 sandbox 场景下，如果你能运行 CLI 并写入平台 Secret，不要让我本地运行 bootstrap 脚本来粘贴 `CLINK_WEBHOOK_SIGNING_KEY`；必须用 `clink webhook endpoint ensure --save-secret --show-secret --json` 获取 signing key 并自动写入平台后端 Secret。只有平台不允许 agent 写 Secret 时，才把这一步列为明确的权限阻塞。
