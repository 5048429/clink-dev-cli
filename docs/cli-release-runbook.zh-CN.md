...
这份文档用于指导人或 AI agent 迭代 `clink-dev-cli`，避免“功能已经推到某个远程分支，但用户/低代码平台安装默认 GitHub 包时仍拿到旧 CLI”的问题。

核心原则：

- 用户和接入提示词默认使用 `npm install -g github:5048429/clink-dev-cli`。
...
node dist/index.js dashboard webhook ensure --help
```
...
clink dashboard webhook ensure --save-secret --show-secret
```
...
git commit -m "Bump clink-dev-cli to <new-version>"
```
...
创建临时目录，从 GitHub 默认分支安装：

```bash
tmp="$(mktemp -d)"
cd "$tmp"
npm install github:5048429/clink-dev-cli
./node_modules/.bin/clink --version
./node_modules/.bin/clink auth secret set --help
./node_modules/.bin/clink dashboard webhook ensure --help
```
...
.\node_modules\.bin\clink.cmd dashboard webhook ensure --help
```

再验证 tag 安装：

```bash
npm install github:5048429/clink-dev-cli#v<new-version>
```
...
原因：`github:5048429/clink-dev-cli` 默认安装 GitHub 默认分支 `main`。功能只在其他分支时，普通安装不会拿到。
