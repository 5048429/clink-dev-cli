...
export function supportedImageMimeTypes() {
    return Array.from(SUPPORTED_IMAGE_MIME_TYPES);
}
export async function inspectImageFile(imageFile, options) {
...
export async function inspectImageUrl(imageUrl) {
...
export async function loadImageUploadAsset(source) {
    if (source.kind === "file") {
...
export function createImageUploadFormFromAsset(asset) {
    const form = new FormData();
    const arrayBuffer = asset.buffer.buffer.slice(asset.buffer.byteOffset, asset.buffer.byteOffset + asset.buffer.byteLength);
...
function resolveCandidateRoots(options) {
    ...
    if (options.projectRoot)
        roots.push(resolve(options.projectRoot));
    if (options.publicDir) {
        roots.push(isAbsolute(options.publicDir)
...
//# sourceMappingURL=catalog-images.js.map