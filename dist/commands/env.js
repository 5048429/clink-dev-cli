...
import { getEnvironmentDefinition, isBuiltInEnvironment, mergeEnvironments, resolveDashboardEndpoints, } from "../environments.js";
...
export function registerEnv(program) {
    ...
    env
        ...
        printResult({ count: environments.length, environments }, outputMode, environments
    ...
    env
        ...
        printResult({ name, environment: definition, configPath: getConfigPath() }, outputMode, [
    ...
    env
        ...
        await writeStoredConfig(stored);
        printResult({ name, removed: true, configPath: getConfigPath() }, outputMode, `Removed environment "${name}" from ${getConfigPath()}`);
    });
}
