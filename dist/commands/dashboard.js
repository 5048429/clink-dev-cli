import { DashboardConsoleClient, extractDashboardApiKeyRecords, extractDashboardMerchantRecords, extractDashboardUserSummary, extractDashboardWebhookRecords, findDashboardPublishableKey, findDashboardSecretKey, findDashboardWebhookByEndpoint, maskDashboardApiKeyRecord, maskDashboardProfile, maskDashboardWebhookRecord, } from "../dashboard-console.js";
...
const DASHBOARD_WEBHOOK_EVENT_CODE_BY_NAME = {
    "order.created": "1",
    "order.succeeded": "2",
    "order.failed": "3",
    "refund.created": "4",
...
export function registerDashboard(program) {
    ...
    dashboard
        ...
        if (config.dryRun) {
            printResult({
                ...
                result,
            }, config.outputMode, "Dashboard whoami dry-run generated. Use --json to view headers.");
            return;
        }
    ...
    apikey
        ...
        if (config.dryRun) {
            printResult({
                ...
                dashboard: maskDashboardProfile(dashboardProfile),
                result,
            }, config.outputMode, "Dashboard API key list dry-run generated. Use --json to view headers.");
            return;
        }
        ...
        printResult({
            ...
            keys: outputRecords,
        }, config.outputMode, outputRecords.length > 0
    ...
    apikey
        ...
        if (config.dryRun) {
            ...
            }, config.outputMode, "Dashboard API key ensure-secret dry-run generated. Use --json to view planned requests.");
            return;
        }
    ...
    merchant
        ...
        if (config.dryRun) {
            printResult({
                ...
                dashboard: maskDashboardProfile(dashboardProfile),
                result,
            }, config.outputMode, "Dashboard merchant list dry-run generated. Use --json to view headers.");
            return;
        }
        ...
        printResult({
            ...
            merchants,
        }, config.outputMode, merchants.length > 0
    ...
    webhook
        ...
        printResult({
            count: events.length,
            events,
        }, config.outputMode, events.map((event) => `${event.name}\t${event.code}`).join("\n"));
    });
    webhook
        .command("list")
        ...
        if (config.dryRun) {
            ...
            }, config.outputMode, "Dashboard webhook list dry-run generated. Use --json to view planned requests.");
            return;
        }
    ...
    webhook
        ...
        .option("--remark <text>", "Dashboard webhook remark/description", "Created by clink-dev-cli")
        .option("--merchant-id <id>", "Merchant ID. If omitted, the CLI resolves it when exactly one merchant is visible.")
        ...
        if (config.dryRun) {
            ...
            }, config.outputMode, "Dashboard webhook create dry-run generated. Use --json to view planned requests.");
            return;
        }
        const merchantContext = await resolveDashboardMerchant(client, options.merchantId);
        const result = await client.createWebhook(buildWebhookPayload(endpoint, eventType, options.remark));
        const record = await enableWebhookIfRequested(client, await resolveWebhookRecordAfterWrite(client, result, merchantContext.merchantId, endpoint), !options.disabled);
    ...
    webhook
        ...
        .action(async function (webhookKeyId, options) {
        ...
        if (config.dryRun) {
            ...
            }, config.outputMode, "Dashboard webhook update dry-run generated. Use --json to view planned requests.");
            return;
        }
        ...
        const record = await enableWebhookIfRequested(client, await resolveWebhookRecordAfterStatusWrite(client, result, webhookKeyId), !options.disabled);
        await saveWebhookSecretIfRequested(config.profile, record, Boolean(options.saveSecret));
        const outputRecord = options.showSecret ? record : maskDashboardWebhookRecord(record);
        printResult({
            profile: config.profile,
    ...
    webhook
        ...
        if (config.dryRun) {
            ...
            }, config.outputMode, "Dashboard webhook ensure dry-run generated. Use --json to view planned requests.");
            return;
        }
        const merchantContext = await resolveDashboardMerchant(client, options.merchantId);
        ...
        if (existing?.webhookKeyId) {
            const shouldUpdate = !sameWebhookEventType(existing.eventType, eventType) ||
    ...
    webhook
        ...
        .action(async function (webhookKeyId) {
...
function requireDashboardProfile(profile) {
    if (!profile?.accessToken || !profile.clientId || !profile.baseUrl) {
        throw new Error("Missing Dashboard Console token. Current official Secret Key API coverage does not include Dashboard webhook management; use public API commands with CLINK_SECRET_KEY where available, or run clink login for Dashboard-only commands.");
    }
    return profile;
}
...
function formatWebhookStatus(status) {
    if (String(status) === DASHBOARD_WEBHOOK_STATUS_ACTIVE)
        return "active";
    if (String(status) === DASHBOARD_WEBHOOK_STATUS_DISABLED)
        return "disabled";
...
async function updateWebhookEnabledStatus(command, webhookKeyId, enabled) {
    ...
    if (config.dryRun) {
        ...
        }, config.outputMode, `Dashboard webhook ${enabled ? "enable" : "disable"} dry-run generated. Use --json to view planned requests.`);
        return;
    }
    const result = await client.updateWebhookStatus(webhookKeyId, status);
...
async function resolveDashboardMerchant(client, explicitMerchantId) {
    if (explicitMerchantId)
        return { merchantId: explicitMerchantId };
    const result = await client.listMerchants();
...
function parseWebhookEventType(value, allowUnknownEvents) {
    ...
    if (normalizedValue === "all") {
        ...
        throw new Error(`Dashboard currently rejects all webhook events in one endpoint because the eventType string is ${allEventType.length} characters. ` +
    ...
    if (eventType.length > DASHBOARD_WEBHOOK_EVENT_TYPE_MAX_LENGTH) {
        throw new Error(`Dashboard currently rejects webhook eventType strings longer than ${DASHBOARD_WEBHOOK_EVENT_TYPE_MAX_LENGTH} characters. ` +
...
function toDashboardWebhookEventCode(event) {
    if (isWebhookEventCode(event))
        return event;
    const code = DASHBOARD_WEBHOOK_EVENT_CODE_BY_NAME[event];
    if (code)
        return code;
    throw new Error(`Dashboard webhook event "${event}" does not have a known numeric code in this CLI build. ` +
...
async function enableWebhookIfRequested(client, record, enabled) {
    if (!enabled || String(record.status) === DASHBOARD_WEBHOOK_STATUS_ACTIVE)
        return record;
...
async function resolveWebhookRecordAfterStatusWrite(client, raw, webhookKeyId) {
    ...
    if (direct?.webhookKeyId || direct?.endpoint)
        return direct;
    try {
